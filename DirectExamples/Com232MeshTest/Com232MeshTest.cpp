...
// Com232MeshTest.cpp
//
// SINGLE-PROCESS, TWO-HUB serial CONNECTION probe using P2PeerCon232.
//
// Mirrors DmxMeshTest exactly, with one difference: the transport is
// P2PeerCon232 (RS-232 serial) instead of P2PeerConDmx. The two hubs sit
// on the two ends of a com0com virtual null-modem pair:
//
//   SERVER hub -- P2PeerCon232::ServiceFactory(..., COM5)   [listens]
//   CLIENT hub -- P2PeerCon232::ClientFactory (..., COM6)   [dials]
//
...
// connection.
//
// PREREQUISITE: a com0com pair COM5<->COM6 (setupc install PortName=COM5
// PortName=COM6). The server MUST be armed before the client dials --
// WaitCommEvent only reports events raised after SetCommMask, so login
// bytes that all arrive before the arm would go unnoticed (the startup
// Sleep below guarantees the ordering, as PeerNetwork's listen gate does
...
static int __cdecl AssertReportHook(int nReportType, char* szMsg, int* pnRet)
{
    if (nReportType == _CRT_ASSERT)
    {
        fflush(stdout);
        fprintf(stderr,
                "\n=== ASSERT TRIPPED (single-process 232 mesh) ===\n%s\n"
                "=== interpreting as: serial connect FAILED ===\n",
...
// =========================================================================
// Com232MeshHub -- same instrumented P2PeerHub shape DmxMeshTest uses.
// =========================================================================
...
protected:
                                     ...
                                     P2Psize_t   iSize) override
    {
        ...
        {
            wprintf(L"[CLIENT] Login ack from '%s' - serial connection ready.\n",
                    strThatP2Paddr);
            PostTestMessage();
            m_bSent = true;
        }
...
int main(int /*argc*/, char* /*argv*/[])
{
    ...
    wprintf(L"=== Com232MeshTest - single-process, two-hub serial probe ===\n");
    wprintf(L"Ports   : server COM%d  <->  client COM%d (com0com pair)\n\n",
    ...
    LogAt(L"SERVER", L"service serial connection posted");

    // Let the server pump run Listen()/Accept() so WaitCommEvent(EV_RXCHAR)
    // is armed BEFORE the client's login bytes hit the wire (events raised
    // before SetCommMask are not reported).
    Sleep(750);
    ...
    LogAt(L"CLIENT", L"client serial connection posted");

    // ---- Wait for the round trip -----------------------------------------
    LogAt(L"MAIN", L"waiting up to 15s for login-ack + BCast delivery...");
    DWORD dwResult = WaitForSingleObject(g_hDoneEvent, 15000);

    int nExit;
    if (dwResult == WAIT_OBJECT_0)
    {
        LogAt(L"MAIN", L"SUCCESS - server received the client's BCast over RS-232");
        nExit = 0;
    }
    else
    {
